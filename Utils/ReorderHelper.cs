...
namespace BetterModManager.Utils
{
    public static class ReorderHelper
    {
        ...
        public static int IncOrDec(int srcIdx, bool isInc)
        {
            ...
            dstIdx = Clamp(dstIdx, PinManager.IsPinned(GetModInfo(srcIdx)));
            // ReorderImpl 会判断 dstIdx 是否在该 Mod 的合法区间内
            // 如果越界（比如普通 Mod 想移入置顶区），ReorderImpl 会返回 false，操作无效
            bool success = ReorderImpl(srcIdx, dstIdx);

            ModLogger.Info($"Mod '{GetName(srcIdx)}' {(isInc ? "下移" : "上移")} " +
                           $"{(success ? "成功" : "被边界拦截")}");

            return success ? dstIdx : srcIdx;
        }
